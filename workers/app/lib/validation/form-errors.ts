...
export type StandardPathSegment = { readonly key: string } | string | number;

const pathSegmentSchema = v.union([v.string(), v.number(), v.object({ key: v.string() })]);
...
export function toFieldErrorsFromIssues<T extends object>(
  ...
  resolveKey: (pathTail: StandardPathSegment | undefined) => (keyof T & string) | null,
) {
