...
- Enabled every generic rule from the requested install policy at `error`, including `oxc/no-accumulating-spread`.
...
- `anti-slop/require-readable-spacing` and its vendored `vendor/eslint-stylistic` tree were removed locally; blank-line policy is delegated to `vp fmt` instead.
