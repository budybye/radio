...
└──────────────────────┬──────────────────────────────────────┘
         ...
         ▼             ▼             ▼
┌─────────────────┐  ┌──────────────────┐  ┌───────────────────────────────┐
│  http://mpd:8000 │  │ http://radio-app:│  │  tcp://radio-mpd:6600         │
│  HTTP ストリーム  │  │ 5173             │  │  MPD 制御ポート（内部のみ）    │
│  （リスナー向け） │  │ Web UI SPA       │  │  （管理者・クライアント向け）  │
└────────┬────────┘  └──────────────────┘  └───────────────────────────────┘
         │                                          ▲
         │         ┌─────────────────────────────────┘
         │         │
...
- **コンテナ単一責任**: MPD コンテナは音楽再生・ストリーミングのみ。Tunnel / app は独立コンテナで分離
...
### `app/Dockerfile`
- **責任**: Web UI（SPA）実行環境の構築（Node.js + Vite + Hono）
- **依存**: `node:slim` ベースイメージ、`app/` ソースコード
- **公開インターフェース**: ポート 5173（HTTP）— `/api` で MPD 制御、`/` で SPA 配信

### `compose.yaml`
- **責任**: マルチコンテナ構成の定義（MPD + app + Tunnel + ボリューム）
- **依存**: `Dockerfile`（MPD サービスのビルド）、`app/Dockerfile`（Web UI のビルド）、`.env`（TUNNEL_TOKEN）
